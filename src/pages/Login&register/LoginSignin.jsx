...
export const LoginSignin = () => {
  ...
  const handleChanges = (e) => {
    ...
    if (name === "contact_number") {
      ...
      FinalData = { ...inputedValues, [name]: truncatedValue }
    } else {
  ...
  const loginNow = () => {
    if (!Errors?.email && !Errors?.password) {
      let CheckingFields = false
      if (inputedValues?.googleVerified) {
        if (inputedValues?.contact_number) {
          CheckingFields = true
        } else {
          CheckingFields = false
        }
      } else {
        console.log(inputedValues?.contact_number, inputedValues?.email, inputedValues?.password)
        if (inputedValues?.contact_number && inputedValues?.email && inputedValues?.password) {
          console.log(inputedValues?.contact_number, inputedValues?.email, inputedValues?.password)
          CheckingFields = true
        } else {
          CheckingFields = false
  ...
  const checkErrors = () => {
    ...
    if (FrgtState?.password && FrgtState?.confirmPwd && FrgtState.password !== FrgtState.confirmPwd) {
    ...
    if (inputedValues?.contact_no && !PhonePattern.test(inputedValues?.contact_no)) {
  ...
  const nextVerification = (data) => {
    if (data?.back) {
      setFrgtState({ ...FrgtState, emailConfirm: false, otpVerification: false })
    } else {
      if (FrgtState?.email) {
        setloading(true)
        const data = {
        ...
        <div className="flex" style={{ alignItems: 'center', justifyContent: 'center' }}>
          <div className="login-form flex">
            <div className="flex input-box-section">
              ...
              <div>
                <div className='LoginPageGoogleAuthBtnAlignDiv'>
                  ...
                  <h4 onClick={() => { resetAuth() }}>OR</h4>
                </div>
              </div>
              <div>
                <h4>Email</h4>
                <input disabled={inputedValues?.googleVerified && inputedValues?.email ? true : false} name='email' value={inputedValues?.email} onChange={handleChanges} type="email" placeholder='example@gmail.com' />
              </div>
              <div>
                <h4>Phone Number</h4>
                <input onKeyDown={handleKeyPress} name='contact_number' value={inputedValues?.contact_number} onChange={handleChanges} type="number" />
              </div>
              {!inputedValues?.googleVerified ?
                <div>
                  <h4>Password</h4>
                  <div
                    style={{
                  ...
                  >
                    <input
                      maxLength={50}
                      name='password'
                      value={inputedValues?.password} onChange={handleChanges}
                      style={{
                        margin: 0,
                        position: "absolute",
                        top: "0",
            ...
            <div className="login-button-section flex">
              <button onClick={() => { navigate("/registerlanding") }} className="flex">
          ...
          <div className='login-InnerModal'>
            {
              loading ?
                <InnerLoader />
                :
                FrgtState?.email && FrgtState?.emailConfirm || FrgtState?.otpVerification ?
                  FrgtState?.otpVerification ?
                    <div className='login-InnerModal-Change-pwd'>
                      <div className='login-InnerModal-close'>
                        <div style={{ cursor: "pointer" }} onClick={() => { nextVerification({ back: true }) }}>
                      ...
                      <div>
                        ...
                        >
                          <input
                            maxLength={50}
                            name='password'
                            value={FrgtState?.password} onChange={updatePassword}
                            style={{
                              margin: 0,
                              position: "absolute",
                              top: "0",
                      ...
                      <div>
                        ...
                        >
                          <input
                            maxLength={50}
                            name='confirmPwd'
                            value={FrgtState?.confirmPwd} onChange={updatePassword}
                            style={{
                              margin: 0,
                              position: "absolute",
                              top: "0",
                      ...
                      <div className='ogin-InnerModal-resetPass-NextDiv'>
                        <button className='loginBUttonStyle' onClick={ResetDone}>Done</button>
                      </div>
                    </div>
                    :
                    <>
                      ...
                      <div className='login-InnerModal-Inputs'>
                        <div className='login-InnerModal-Inputs-Align'>
                          <MuiOtpInput value={otp?.userOtp} onChange={handleChange} />
                        </div>
                      </div>
                      <p className='login-InnerModal-Inputs-Ptag'>Didin't receive a code? <a onClick={nextVerification} >Resent</a></p>
                      <div className='login-InnerModal-Button'>
                        <button className='loginBUttonStyle' onClick={VerifyOtp} >Verify Code </button>
                      </div>
                    </>
                  :
                  <div className='ogin-InnerModal-resetPass'>
                    ...
                    <input placeholder='example@gmail.com' value={FrgtState?.email} onChange={frgtInputData} name='email' type="email" />
                    <div className='ogin-InnerModal-resetPass-NextDiv'>
                      <button className='loginBUttonStyle' onClick={nextVerification}>Next</button>
                    </div>
                  </div>
            }
          </div>
